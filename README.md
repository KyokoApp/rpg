...
- **Gaya kartun / cel-shaded (lembut)** — gradien smoothstep mid-tone (tidak gelap pekat, tidak terang berlebih), ambient netral agar **warna tidak bentrok**, rim light tipis, dan **outline tipis cokelat hangat** (bukan hitam tebal). Palet satu famili (batu pirus-hijau hangat + atap giok + tanah pastel) supaya keseluruhan menyatu.
- **Rumput mulus** — gradien **akar hijau gelap → ujung hijau terang** tanpa banding (posterisasi dihapus, kecerahan per helai cuma 8%), **bilah melengkung miring** dengan arah condong per helai + **efek sepoi angin** (gelombang berjalan + gust + flutter). Tanah dijadikan hijau gelap senada akar rumput supaya keduanya menyatu.
- **Pencahayaan & bayangan halus** — shadow map PCF-soft 2048² real-time; **karakter & benteng melempar bayangan lunak ke tanah**. Material toon diganti **`MeshToonMaterial` bawaan + gradient map 4 tingkat** (sebelumnya `ShaderMaterial` kustom yang tidak menerima shadow map — itulah sebabnya bayangan tidak muncul). **Satu arah matahari** (`SUN_DIR`) jadi sumber kebenaran: cahaya directional, piringan/halo matahari yang terlihat, shading toon, dan arah bayangan semuanya konsisten — bayangan jatuh berlawanan dengan posisi matahari.
- **Awan 2D "digambar lembut"** — sprite kanvas dengan tepi blur radial, bukan gumpal 3D; hanyut pelan sambil naik-turun, bergaya lukisan anime.
- **Karakter beranimasi (pengganti bola)** — karakter toon from-scratch: kepala + rambut + mata, torso, lengan & kaki dengan langkah berlari, ayunan lengan, bobbing, menghadap arah gerak, dan bantuan bayangan saat melompat. (Model Drive `.fbx/.blend` tidak bisa diproses — butuh `.glb/.gltf`; lihat catatan di bawah.)
- **Auto fullscreen + landscape** — mengetuk tombol mulai otomatis meminta fullscreen (fallback `webkitRequestFullscreen`) lalu mencoba `screen.orientation.lock('landscape')`; keduanya opsional dan gagal dengan aman.
