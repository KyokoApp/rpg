 ...
 try{
  for(const mobile of (process.env.RPG_MOBILE_ONLY? [true]:[true,false])){
   ...
   await page.route('**/',async route=>{
    ...
    if(process.env.RPG_FAST_SMOKE==='1')body=body.replace('const GRASS_N=isTouch?30000:130000','const GRASS_N=1500');
    body=body.replace('animate();\naddEventListener','window.qa={CHAR,player,ATTACK,renderer,tryAttack,startGame,pauseGame};\nanimate();\naddEventListener');
   ...
   await page.locator('#mapPanel [data-close-panel]').click();
   await page.waitForFunction(()=>document.body.classList.contains('playing'));
   await page.waitForFunction(()=>getComputedStyle(document.getElementById('hud')).opacity==='1');
